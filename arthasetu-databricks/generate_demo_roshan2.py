"""
Generate fake bank statement and loan slip PDFs for unstable demo user Roshan Pandey 2,
...
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "output", "demo_roshan2")
os.makedirs(OUTPUT_DIR, exist_ok=True)

ACCOUNT_HOLDER = "Roshan Pandey"
ACCOUNT_NUMBER = "3201 4587 7841"
IFSC = "SBIN0012345"
...
# ─── Generate unstable transaction data ──────────────────────────────────────

def generate_transactions():
    """
    Generate ~75 transactions for an UNSTABLE farmer:
    - Irregular income: some months very low, some decent
    - Many small credits (< 5000) — fragmented, informal income
    - Some large debits (> 10000) — sudden big expenses
    - Balance dips near zero occasionally
    - Higher cash usage
    """
    transactions = []
    start = datetime(2023, 7, 1)
    balance = 8200.0  # low opening balance

    descriptions_debit_small = [
        ("Groceries", "UPI"), ("Mobile Recharge", "UPI"), ("Electricity Bill", "UPI"),
        ("Pharmacy", "UPI"), ("Dairy Milk", "UPI"), ("Local Store", "cash"),
        ("Tea Stall", "cash"), ("Auto Fare", "cash"),
    ]
    descriptions_debit_large = [
        ("Tractor Rental", "cash"), ("Fertilizer Purchase", "cash"),
        ("Pesticide Bulk Buy", "cash"), ("Hospital Bill", "UPI"),
        ("Veterinary Emergency", "cash"), ("Wedding Contribution", "cash"),
    ]
    descriptions_credit_small = [
        ("Daily Labour Wage", "cash"), ("Milk Sale", "cash"),
        ("Vegetable Sale - Local", "cash"), ("Neighbour Repayment", "UPI"),
    ]
    descriptions_credit_large = [
        ("Mandi Receipt", "bank transfer"),
        ("Agricultural Produce Sale", "bank transfer"),
        ("PM-KISAN Installment", "bank transfer"),
    ]

    # Monthly pattern for unstable farmer — income varies wildly
    monthly_profiles = [
        {"large_credits": 1, "small_credits": 3, "large_debits": 1, "small_debits": 8},   # Jul - ok
        {"large_credits": 0, "small_credits": 2, "large_debits": 1, "small_debits": 7},   # Aug - bad month
        {"large_credits": 1, "small_credits": 4, "large_debits": 2, "small_debits": 9},   # Sep - harvest but heavy spending
        {"large_credits": 0, "small_credits": 1, "large_debits": 1, "small_debits": 6},   # Oct - very lean
        {"large_credits": 1, "small_credits": 3, "large_debits": 2, "small_debits": 10},  # Nov - some recovery + expenses
        {"large_credits": 1, "small_credits": 2, "large_debits": 1, "small_debits": 7},   # Dec - moderate
    ]

    for month_offset, profile in enumerate(monthly_profiles):
        month_start = start + timedelta(days=month_offset * 30)

        # Large credits (> 5000)
        for _ in range(profile["large_credits"]):
            day_offset = random.randint(1, 28)
            txn_date = month_start + timedelta(days=day_offset)
            desc, channel = random.choice(descriptions_credit_large)
            amount = round(random.uniform(8000, 22000), 2)
            balance = round(balance + amount, 2)
            transactions.append({"date": txn_date, "type": "credit", "amount": amount,
                                 "balance": balance, "description": desc, "channel": channel})

        # Small credits (< 5000) — fragmented informal income
        for _ in range(profile["small_credits"]):
            day_offset = random.randint(0, 29)
            txn_date = month_start + timedelta(days=day_offset)
            desc, channel = random.choice(descriptions_credit_small)
        ...
        # Large debits (> 10000)
        for _ in range(profile["large_debits"]):
            day_offset = random.randint(0, 29)
            txn_date = month_start + timedelta(days=day_offset)
            desc, channel = random.choice(descriptions_debit_large)
            amount = round(random.uniform(10500, 25000), 2)
            balance = round(balance - amount, 2)
            if balance < 200:
                balance = round(balance + random.uniform(500, 2000), 2)
            transactions.append({"date": txn_date, "type": "debit", "amount": amount,
                                 "balance": balance, "description": desc, "channel": channel})

        # Small debits
        for _ in range(profile["small_debits"]):
            day_offset = random.randint(0, 29)
            txn_date = month_start + timedelta(days=day_offset)
            desc, channel = random.choice(descriptions_debit_small)
            amount = round(random.uniform(50, 1800), 2)
            balance = round(balance - amount, 2)
            if balance < 200:
                balance = round(balance + random.uniform(500, 2000), 2)
            transactions.append({"date": txn_date, "type": "debit", "amount": amount,
...
# ─── Generate loan data (unstable) ───────────────────────────────────────────

def generate_loans():
    """3 loans — overextended, multiple missed payments, one near-default."""
    return [
        {
            "loan_id": "RP2_L01",
            "loan_amount": 80000,
            "loan_start_date": "2023-04-10",
            "loan_due_date": "2023-12-10",
            "interest_rate": 14.5,
            "repayment_type": "seasonal",
            "amount_repaid": 22000.0,
            "outstanding_amount": 66600.0,
            "missed_payments": 3,
            "days_past_due": 28,
            "default_flag": 0,
            "purpose": "Kharif crop inputs and tractor rental",
        },
        {
            "loan_id": "RP2_L02",
            "loan_amount": 45000,
            "loan_start_date": "2023-07-20",
            "loan_due_date": "2024-03-20",
            "interest_rate": 16.0,
            "repayment_type": "bullet",
            "amount_repaid": 0.0,
            "outstanding_amount": 48600.0,
            "missed_payments": 4,
            "days_past_due": 35,
            "default_flag": 0,
            "purpose": "Medical emergency — family illness",
        },
        {
            "loan_id": "RP2_L03",
            "loan_amount": 30000,
            "loan_start_date": "2023-10-05",
            "loan_due_date": "2024-04-05",
            "interest_rate": 17.25,
            "repayment_type": "EMI",
            "amount_repaid": 5000.0,
            "outstanding_amount": 27587.50,
            "missed_payments": 2,
            "days_past_due": 18,
            "default_flag": 0,
            "purpose": "Debt consolidation — repay informal lender",
        },
    ]
...
def create_bank_statement_pdf(transactions, filepath):
    ...
    for txn in transactions:
        ...
        table_data.append([
            txn["date"].strftime("%Y-%m-%d"), txn["description"], txn["channel"],
            debit, credit, f"{txn['balance']:,.2f}",
        ])
    ...
    summary_data = [
        ["Total Credits:", f"₹{total_credits:,.2f}", "Total Debits:", f"₹{total_debits:,.2f}"],
...
def create_loan_slip_pdf(loans, filepath):
    ...
    borrower_data = [
        ...
        ["Occupation:", "Farmer (Marginal - 1.5 acres)", "Crop:", "Rice, Mustard"],
    ]
    ...
    elements.append(Spacer(1, 3*mm))
    totals_data = [
        ["Total Loan Amount:", f"₹{total_loan:,}", "Total Repaid:", f"₹{total_repaid:,.2f}"],
        ["Total Outstanding:", f"₹{total_outstanding:,.2f}", "Number of Loans:", str(len(loans))],
    ]
...
def compute_behavioral_features(transactions):
    ...
    # Forward-fill credit_debit_time_gap (matches training code)
...
def save_loans_csv(loans, filepath):
    with open(filepath, "w", newline="") as f:
        ...
        for l in loans:
            writer.writerow({
                "user_id": "ROSHAN02", "loan_id": l["loan_id"],
                "loan_amount": l["loan_amount"], "loan_start_date": l["loan_start_date"],
                "loan_due_date": l["loan_due_date"], "interest_rate": l["interest_rate"],
                "repayment_type": l["repayment_type"], "amount_repaid": l["amount_repaid"],
                "outstanding_amount": l["outstanding_amount"],
                "missed_payments": l["missed_payments"], "days_past_due": l["days_past_due"],
...
FEATURE_IMPORTANCE = {
    "credit_debit_time_gap": 0.2805, "missed_payment_count": 0.0713,
    "debt_to_income_ratio": 0.0542, "num_loans": 0.0369,
    "small_credit_count": 0.0341, "cash_vs_upi_ratio": 0.0330,
    "avg_days_past_due": 0.0310, "balance_std": 0.0295,
    "inflow_variance": 0.0280, "outstanding_to_balance_ratio": 0.0270,
    "repayment_ratio": 0.0260, "total_outstanding": 0.0250,
    "monthly_inflow": 0.0240, "avg_balance": 0.0230,
    "max_balance": 0.0220, "debt_growth_rate": 0.0210,
    "total_loan_amount": 0.0200, "avg_interest_rate": 0.0190,
    "inflow_outflow_ratio": 0.0180, "monthly_outflow": 0.0170,
    "min_balance": 0.0160, "total_transactions": 0.0150,
    "txn_frequency": 0.0140, "avg_transaction_amount": 0.0130,
    "large_debit_count": 0.0120, "consecutive_zero_balance_days": 0.0110,
}
...
def create_diagnostic_visualization(transactions, features, predictions, user_id, name, filepath):
    ...
    creds = [t for t in transactions if t["type"] == "credit"]
    debs = [t for t in transactions if t["type"] == "debit"]
    if creds:
        c_d, c_b = [t["date"] for t in creds], [t["balance"] for t in creds]
        c_a = np.array([t["amount"] for t in creds])
        ax_time.scatter(c_d, c_b, s=(c_a / c_a.max()) * 400 + 40, color="#27AE60",
                        alpha=0.7, zorder=2, label="Credit", edgecolor="white", lw=0.5)
    if debs:
        d_d, d_b = [t["date"] for t in debs], [t["balance"] for t in debs]
        d_a = np.array([t["amount"] for t in debs])
        ax_time.scatter(d_d, d_b, s=(d_a / d_a.max()) * 400 + 40, color="#E74C3C",
                        alpha=0.7, zorder=2, label="Debit", edgecolor="white", lw=0.5)

    ax_time.axhline(0, color="black", lw=1, ls="--", alpha=0.4)
    behav = predictions.get("behavioral_class", "?").upper()
    ax_time.set_title(f"Financial Timeline: {name} ({user_id}) — {behav}",
    ...
    # ── Panel 2: Top 5 Risk Drivers ──
    all_cols = list(POP_STATS.keys())
    reasons = []
    for feat in all_cols:
        val = features.get(feat, 0)
        m = POP_STATS[feat]["mean"]
        s = POP_STATS[feat]["std"]
        z = (val - m) / s if s > 0 else 0
        imp = FEATURE_IMPORTANCE.get(feat, 0.01)
        reasons.append({"feature": feat, "score": abs(z) * imp, "val": val, "mean": m, "z": z})
    top = sorted(reasons, key=lambda x: x["score"], reverse=True)[:5]

    names_list = [r["feature"].replace("_", " ").title() for r in top]
    zs = [r["z"] for r in top]
    bar_c = ["#E74C3C" if z > 0 else "#3498DB" for z in zs]
    ax_anom.barh(range(len(names_list)), zs, color=bar_c, edgecolor="white")
    ax_anom.set_yticks(range(len(names_list)))
    ax_anom.set_yticklabels(names_list, fontsize=11)
    ...
    for i, r in enumerate(top):
        ...
        info += (f"{i+1}. {r['feature'].replace('_',' ').title()}:\n"
                 f"   Actual: {r['val']:.1f}  |  Pop Mean: {r['mean']:.1f}\n"
                 f"   {d} than average (Z: {r['z']:+.2f})\n")
...
def main():
    ...
    print("=" * 60)
    print(f"  Generating demo data for: {ACCOUNT_HOLDER} (UNSTABLE)")
    print("=" * 60)

    transactions = generate_transactions()
    loans = generate_loans()

    save_transactions_csv(transactions, os.path.join(OUTPUT_DIR, "roshan2_transactions.csv"))
    save_loans_csv(loans, os.path.join(OUTPUT_DIR, "roshan2_loans.csv"))

    create_bank_statement_pdf(transactions, os.path.join(OUTPUT_DIR, "Roshan_Pandey_2_Bank_Statement.pdf"))
    create_loan_slip_pdf(loans, os.path.join(OUTPUT_DIR, "Roshan_Pandey_2_Loan_Slip.pdf"))
    ...
    features_path = os.path.join(OUTPUT_DIR, "roshan2_features.json")
    with open(features_path, "w") as f:
        json.dump({"user_id": "ROSHAN02", "name": ACCOUNT_HOLDER, "features": all_features}, f, indent=2)
    print(f"\nFeatures saved: {features_path}")

    # ── Call Databricks APIs ──────────────────────────────────────────────────
    ...
    print("\n" + "=" * 60)
    print("  Endpoint 1: Behavioral Classifier (Random Forest)")
    print("=" * 60)
    resp1 = requests.post(
        ...
        json={"dataframe_split": {
            ...
            "data": [[all_features[c] for c in BEHAVIORAL_COLS]],
        }},
        timeout=90,
    )
    ...
    classification = result1.get("predictions", [None])[0]
    print(f"\n  >> Roshan Pandey 2 is classified as: {classification}")

    print("\n" + "=" * 60)
    print("  Endpoint 2: Default Predictor (XGBoost)")
    print("=" * 60)
    resp2 = requests.post(
        ...
        json={"dataframe_split": {
            ...
            "data": [[all_features[c] for c in DEFAULT_COLS]],
        }},
        timeout=90,
    )
    ...
    prob = result2.get("predictions", [None])[0]
    if prob is not None:
        risk_tier = "low_risk" if prob < 0.3 else ("medium_risk" if prob < 0.6 else "high_risk")
        print(f"\n  >> Default Probability: {prob:.4f} ({prob*100:.2f}%)")
        print(f"  >> Risk Tier: {risk_tier}")

    print("\n" + "=" * 60)
    print("  Summary for Roshan Pandey 2 (UNSTABLE)")
    print("=" * 60)
    print(f"  Behavioral Class : {classification}")
    print(f"  Default Prob     : {prob:.4f}")
    print(f"  Risk Tier        : {risk_tier}")
    print("=" * 60)

    predictions = {
        "behavioral_class": classification,
        "default_probability": prob,
        "risk_tier": risk_tier,
    }

    result = {
        "user_id": "ROSHAN02", "name": ACCOUNT_HOLDER,
        "features": all_features,
        "predictions": predictions,
    }
    result_path = os.path.join(OUTPUT_DIR, "roshan2_prediction_result.json")
    ...
    create_diagnostic_visualization(
        ...
        user_id="ROSHAN02", name=ACCOUNT_HOLDER,
        filepath=os.path.join(OUTPUT_DIR, "Roshan_Pandey_2_Diagnostic.png"),
    )
